...
const AddTopics = ({ categoryID, setResetContent }) => {
    ...
    const [openAddTopics, setOpenAddTopics] = useState(false);
    ...
    const handleSubmit = () => {
        ...
        if (Object.keys(validationResults).length === 0) {
            let bodyData = {
                ...
                belongToCategory: categoryID
            }
            try {
                axios
                    .post(`${process.env.REACT_APP_BASE_URL + "/api/topic"}`, bodyData, {
                        headers: {
                            Authorization: `Bearer ${JSON.parse(localStorage.getItem('token'))}`,
                        },
                    })
                    .then((res) => {
                        toast("Topic Added Successfully", "successToast");
                        setOpenAddTopics(false);
                        setFormData({
                            name: "",
                            description: "",
                        });
                    ...
                    .catch((err) => {
                        toast("Error in Adding Topic", "errorToast");
                    });
            } catch (err) {
    ...
    return (
        <>
            <button onClick={() => setOpenAddTopics(true)} className='primary-button'>Add Topic</button>
            <ModalBox
                visible={openAddTopics}
                visibleFunction={setOpenAddTopics}
                heading={"Add Topic"}
                body={
                ...
                footerButtons={[
                    ...
                    },
                    {
                        text: "Close",
                        type: "danger",
                        onClick: () => { setOpenAddTopics(false); handleErrors({}) }
                    }
                ]}
            />
        </>
